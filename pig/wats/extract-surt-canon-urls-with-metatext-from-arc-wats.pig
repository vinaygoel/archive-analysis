...
Orig = LOAD '$I_WATS_DIR' USING org.archive.hadoop.ArchiveJSONViewLoader('Envelope.ARC-Header-Metadata.Target-URI','Envelope.ARC-Header-Metadata.Date','Envelope.Payload-Metadata.HTTP-Response-Metadata.Response-Message.Status','Envelope.Payload-Metadata.HTTP-Response-Metadata.HTML-Metadata.Head.Title','Envelope.Payload-Metadata.HTTP-Response-Metadata.HTML-Metadata.Head.@Metas.{content,name}') AS (src:chararray,timestamp:chararray,status:chararray,title:chararray,metacontent:chararray, metaname:chararray);
